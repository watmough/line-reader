...
struct reader {
  ...
  // with the next line.
  std::vector<char>::iterator getline() {
    // set current line to pos
    _line = _pos;
    // at end of buffer? no more lines, return nullptr
    if (_line == _end)
      return static_cast<std::vector<char>::iterator>(nullptr);
...
int main() {
  ...
  reader linereader("./las-files/1052127278.las");

  while (linereader.hasline()) {
    auto line = linereader.getline();
